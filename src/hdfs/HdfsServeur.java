...
public class HdfsServeur {
	...
	/**
	 * Permet de créer un noeud (serveur) hdfs
	 * commande : java hdfsserveur port
	 * @param args
	 * @throws IOException 
	...
	public static void main(String[] args) throws IOException, ClassNotFoundException {
		...
		@SuppressWarnings("resource") //libérée automatiquement à la fin du process
		...
		try {
            ...
            } catch(FileAlreadyExistsException e) {} //besoin de rien besoin de tout
            HdfsServeur.prefix = HdfsServeur.prefix+"/files-"+HdfsServeur.hstname;
			Files.createDirectory(Paths.get(HdfsServeur.prefix));
		} catch(FileAlreadyExistsException e) {} //le fichier est deja cree besoin de rien
		...
		while (true) {
			...
			switch(cmd) {
			case "write":
				//ce genre de try sert à éviter de faire crash le serveur par une mauvaise manip du client
				try {
					write(ois);
				} catch(Exception e) {
					e.printStackTrace();
	...
	public static void delete(ObjectInputStream ois) throws IOException, ClassNotFoundException {
		...
		try {
			...
			System.out.println(HdfsServeur.prefixlog + "fichier introuvable ...");
		}
	}
